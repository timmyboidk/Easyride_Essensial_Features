...
public class MatchingController {
    ...
    public ApiResponse<DriverAssignedEventDto> matchDriver(@Valid @RequestBody MatchRequestDto matchRequestDto) {
        ...
        if (assignmentResult != null) {
            log.info("Manual match successful for order {}: driver {}", matchRequestDto.getOrderId(), assignmentResult.getDriverId());
    ...
    public ApiResponse<String> updateDriverStatus(@PathVariable Long driverId, @Valid @RequestBody DriverStatusUpdateDto statusUpdateDto) {
    ...
    /**
     * Endpoint for a driver to accept a manually offered order or a "grab-able" order.
     * The orderId and driverId (from authenticated principal) are key.
     */
    @PostMapping("/orders/{orderId}/accept")
    public ApiResponse<String> acceptOrder(@PathVariable Long orderId /*, @AuthenticationPrincipal CustomUserDetails driverDetails */) {
        // Long driverId = driverDetails.getId(); // Get driver ID from authenticated principal
        ...
        if (accepted) {
            ...
            log.warn("Order {} could not be accepted by driver {} (e.g., already taken or invalid state)", orderId, driverId);
            return ApiResponse.error(409, "订单无法接受（可能已被分配或状态无效）");
        }
    }

    /**
     * Endpoint for drivers to fetch a list of available orders for manual selection/grabbing.
     */
    @GetMapping("/orders/available")
    public ApiResponse<List<AvailableOrderDto>> getAvailableOrders(/* @RequestParam(required = false) String region, etc. */) {
        // Potentially get driver's current region/preferences from their profile or status
        log.info("Fetching available orders for manual selection.");
        List<AvailableOrderDto> availableOrders = matchingService.getAvailableOrdersForDriver(/* driverId, driverPreferences */);
