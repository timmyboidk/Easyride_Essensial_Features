...
public class GlobalExceptionHandler {
        ...
        public ErrorResponse handleConstraintViolationException(ConstraintViolationException ex) {
                String violationMessages = ex.getConstraintViolations().stream()
                                .map(violation -> String.format("Parameter '%s': %s", violation.getPropertyPath(),
        ...
        public ErrorResponse handleTypeMismatchException(MethodArgumentTypeMismatchException ex) {
                String errorMessage = String.format(
                                ...
                                ex.getRequiredType() != null ? ex.getRequiredType().getSimpleName() : "Not specified");
                log.warn(errorMessage);
                return new ErrorResponse(
                                HttpStatus.BAD_REQUEST.value(),
                                errorMessage,
         ...
         * Avoids these being caught by the final Exception handler, allowing for a more
         * specific status code if needed.
         * Returns HTTP 400 Bad Request.
         */
        @ExceptionHandler(RuntimeException.class)
        @ResponseStatus(HttpStatus.BAD_REQUEST)
        public ErrorResponse handleRuntimeException(RuntimeException ex, WebRequest request) {
                // This handler will now catch any RuntimeException that is NOT a subclass of
                // the more specific ones above.
        ...
        public ErrorResponse handleGlobalException(Exception ex, WebRequest request) {
                ...
                return new ErrorResponse(
                                ...
                                "An unexpected internal server error occurred. Please try again later.", // User-facing
                                                                                                         // message
                                LocalDateTime.now());
        }
}