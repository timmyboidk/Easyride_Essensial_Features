...
-   **Columns**:
    ...
    -   `role` (ENUM('PASSENGER', 'DRIVER', 'ADMIN'), NOT NULL): User role.
...
-   **Columns**:
    ...
    -   `id` (BIGINT, PK): Primary key (Foreign Key to `users.id`).
    ...
    -   `verification_status` (ENUM('PENDING_SUBMISSION', 'PENDING', 'APPROVED', 'REJECTED'), NOT NULL): Verification status.
