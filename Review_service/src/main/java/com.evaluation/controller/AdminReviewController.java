...
public class AdminReviewController {
    ...
    public ApiResponse<EvaluationDTO> performActionOnEvaluation(
            ...
            @Valid @RequestBody AdminReviewActionDto actionDto /*, @AuthenticationPrincipal UserPrincipal adminPrincipal */) {
        // Long adminId = adminPrincipal.getId();
        Long adminId = 0L; // Placeholder for admin ID
        log.info("Admin {} performing action on evaluation {}: {}", adminId, evaluationId, actionDto.getNewEvaluationStatus());
        EvaluationDTO updatedEvaluation = evaluationService.adminUpdateEvaluationStatus(evaluationId, actionDto.getNewEvaluationStatus(), actionDto.getAdminNotes(), adminId);
    ...
    public ApiResponse<ComplaintDTO> performActionOnComplaint(
        ...
        log.info("Admin {} performing action on complaint {}: {}", adminId, complaintId, actionDto.getNewComplaintStatus());
        ComplaintDTO updatedComplaint = complaintService.adminUpdateComplaintStatus(complaintId, actionDto.getNewComplaintStatus(), actionDto.getAdminNotes(), adminId);
